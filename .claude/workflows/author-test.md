...
- Spec under `tests/`
- Page/component code under `pages/` and `pages/components/` when the scenario needs it
...
2. **Plan.** Walk the scenario from entry point through each user-visible step. List touched pages and components. Compare against `pages/` and `pages/components/` to decide what to reuse vs add. Print a short plan (files to create / update) before broad edits.
